...
- The queue was `fakeredis` everywhere, and no automated test had seen a real RQ worker dequeue a real job — Docker Compose was the only place that path ran at all. `tests/test_queue_real_redis.py` and the `queue` CI job now cover it: a real Redis service, the forking `rq.Worker` that production runs, an unmocked analysis, and the run row read back through a fresh session. **Written and not yet observed passing** — the first run of that job is the thing to check. What it will still not cover: concurrency between workers, RQ retries, the hour-long job timeout, and a worker killed mid-EM (which leaves a run stuck in RUNNING, with no reaper).
